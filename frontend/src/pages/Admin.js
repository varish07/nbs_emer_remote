...
import { Loader2, Flag, Users, MessageCircle, Ban, CheckCircle2, AlertTriangle, X, ArrowLeft } from "lucide-react";
      ...
      <div className="px-5 mt-5 flex gap-2 border-b border-[#EBEBEB]">
        {TABS.map((t) => (
          <button
            ...
            onClick={() => setTab(t.id)}
            className={`pb-2 px-3 text-sm font-semibold ${tab === t.id ? "text-[#FF385C] border-b-2 border-[#FF385C]" : "text-[#717171]"}`}
          >
            {t.label}
          </button>
        ))}
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-[#FAFAFA]">
              {openChat.messages.map((m) => {
                ...
                return (
                  <div key={m.id} className="rounded-2xl bg-white border border-[#EBEBEB] p-3">
                    <div className="flex items-center gap-2 mb-1">
